...
This page explains the engine side of the contract: what `ModelEngine` requires, why engines share outputs rather than an implementation, and where the built-in engines stand. [Architecture](architecture.md) covers the two guarantees behind the outcome structure, and the [quickstart](https://heormodel.pedrodelima.com/) introduces the `Outcomes` structure those engines return.
...
A plain function `draws -> Outcomes` works too; the name `ModelFn` refers to that form, and `run_psa` accepts either. The [full pipeline tutorial](https://heormodel.pedrodelima.com/tutorials/full-pipeline.html) runs a decision tree written as a function.
