...
def model(p, strategy):
    rr = {"Standard care": 1.0, "Treatment A": p["rr_a"], "Treatment B": p["rr_b"]}[strategy]
    add_cost = {"Standard care": 0.0, "Treatment A": p["c_a"], "Treatment B": p["c_b"]}[strategy]
...
engine = MarkovModel(states=("Healthy", "Sick", "Dead"),
                     strategies=("Standard care", "Treatment A", "Treatment B"),
...
The `icer_table` orders strategies by cost and reports incremental cost-effectiveness ratios along the efficiency frontier; the `status` column flags dominated (`D`) and extended-dominated (`ED`) strategies. All three strategies sit on the frontier here, with Treatment A entering near 11,000 per quality-adjusted life-year (QALY) and Treatment B near 52,000. `plot_ce_plane` scatters every iteration's incremental cost and effect against standard care, and `plot_ceac` reports the share of iterations in which each strategy has the highest net benefit, across a grid of willingness-to-pay thresholds:
