...
The model tracks four states, Healthy, Sick, Sicker, and Dead, over 75 annual cycles starting at age 25. Four strategies compare standard of care with treatment A (improves the Sick-state utility), treatment B (slows progression from Sick to Sicker), and their combination AB. The `model_fn` function below returns each strategy's transition matrix and per-state payoffs from a single parameter row. Transition rates, not probabilities, are the quantities sampled and adjusted by hazard ratios: a hazard ratio scales a rate multiplicatively, so `hr_S1` and `hr_S2` multiply the death rate before `r2p` converts it to a per-cycle probability, matching how the source article specifies uncertainty.
...
The article's uncertainty distributions for each parameter become a `ParameterSet`; gamma distributions bound rates and costs at zero, beta distributions bound utilities to the unit interval, and log-normal distributions keep hazard ratios positive. From there, `run_psa` and the analysis layer treat this cohort model like any other engine. The run below passes `sequential=True` because 1,000 iterations finish quickly in-process; larger runs default to parallel batches across cores.
...
EVPI is positive at a threshold of 100,000 because Strategy AB, the most expensive option, is the best choice in some draws but not others: the uncertainty in whether it is worth adopting has a real cost, and resolving it would be worth exactly this much per person.
