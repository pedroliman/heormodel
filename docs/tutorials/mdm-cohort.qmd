...
This page reproduces the introductory Sick-Sicker cohort state-transition analysis of Alarid-Escudero and others (2023) with `MarkovModel`, then runs its probabilistic sensitivity analysis through the same pipeline. It assumes the [bring your own outputs tutorial](byo-outputs.qmd). Full script: [`examples/mdm_cohort.py`](https://github.com/pedroliman/heormodel/blob/main/examples/mdm_cohort.py); companion replications: [replication gallery](replication-gallery.qmd).

## The model

Four states (Healthy, Sick, Sicker, Dead) over 75 annual cycles from age 25. Four strategies compare standard of care with treatment A (improves the Sick-state utility), treatment B (slows progression), and their combination AB. The `model_fn` callback returns each strategy's transition matrix and per-state payoffs from a parameter row.
...
One parameter set reproduces the published table exactly: Strategy A is dominated; the frontier runs standard of care, Strategy B (ICER about 73,000 per QALY), Strategy AB (about 126,000 per QALY).
...
The mean frontier matches the deterministic ranking. EVPI is positive because Strategy AB clears the 100,000 threshold in some draws but not others.
