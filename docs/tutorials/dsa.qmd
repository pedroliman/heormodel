...
`heormodel.dsa` builds scenario designs that run through `run_psa` unchanged: a design is a draw matrix whose rows are scenarios instead of random draws, paired with a descriptor naming what each scenario varied. Reusing `run_psa` this way means nothing in the run loop or the `Outcomes` structure needs to change between a probabilistic and a deterministic analysis.
