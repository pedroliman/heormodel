...
fan = {}
for n in sizes:  # the same study, vectorised across the threshold grid
...
Two features look odd until you read them off the model. At a zero threshold EVPI is already about 1,600: health is unpriced, so the choice turns on cost alone, and the uncertain cost `dc` is a saving 31% of the time that perfect information would capture. The EVSI curves start at zero there, since an effect study is worthless when the effect has no value. And nothing peaks: the NB standard deviation `hypot(WTP * SD_Q, SD_C)` grows with the threshold, so the stakes of a wrong choice climb while the chance of one settles near 25%. A model with bounded outcome variance would grow certain at a high threshold instead, its VoI peaking near the tie then decaying.
