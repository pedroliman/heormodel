...
This tutorial shows how to calibrate a model when each run is expensive, by fitting a fast surrogate of the model on a small design and running the inference against the surrogate instead. It trains a Gaussian process on a few dozen model runs, then calibrates twice against it, once with approximate Bayesian computation (ABC) and once with neural posterior estimation, and checks both against a direct ABC run on the model. All three posteriors agree, and the surrogate reaches them with about sixty model runs against the thousands a direct calibration takes. It builds on the [ABC](calibrate-abc.qmd) and [simulation-based inference](calibrate-sbi.qmd) tutorials, walks through [`examples/surrogate_calibration.py`](https://github.com/pedroliman/heormodel/blob/main/examples/surrogate_calibration.py), and needs both extras: `uv pip install 'heormodel[calibration,surrogate]'`.

The first two tutorials each calibrated the model directly and each spent thousands of model runs. A surrogate moves that cost up front into a fixed design: run the model a few dozen times, fit a fast approximation of its output, and let the inference query the approximation without running the model again. The tutorial makes two points. The surrogate is faithful, so inference against it reproduces the direct posterior. And the two inference methods, run on the same surrogate, agree, so the method and the surrogate are independent choices.
...
The surrogate runs the model only on a fixed design, chosen once. Latin hypercube sampling spreads 60 points across the two-parameter box so each parameter's range is evenly covered, which a uniform random draw of the same size would leave patchier. Sixty points is a deliberately small budget, to show how few runs the surrogate needs.
...
A Gaussian process regresses each target on the two parameters. It suits a small design of a smooth, deterministic function: it interpolates the design points and reports its own uncertainty away from them. One Gaussian process is fit per target, each with a squared-exponential correlation and a small noise term for numerical stability.
...
Before trusting the surrogate, check it against model runs that were not in its design. Predicting 300 fresh points and comparing to the model gives the root-mean-square error per target.
...
The error is on the order of 0.0001, far below the sampling error of about 0.015 the calibration works against. At that scale the surrogate is indistinguishable from the model, so inference against it reaches the same posterior. These hold-out runs are a check, not part of the calibration budget.
...
To confirm the surrogate did not distort the answer, run ABC once more against the model itself, the calibration of the first tutorial. This run spends the model budget the surrogate avoids.
...
The surrogate posteriors, from either method, sit on top of the direct posterior and on the truth.
...
The surrogate reached the same posterior on the design's 60 runs, against the thousands the direct calibration spent.

```{python}
print(f"direct ABC model runs: {reference_runs}")
print(f"surrogate model runs:  {design_runs}")
print(f"ratio:                 {reference_runs / design_runs:.0f} times fewer")
```

The ratio is fixed by the design size and does not grow with the inference, so a slower model or a more demanding posterior widens the advantage rather than narrowing it. Two conditions limit the method. The surrogate is reliable only within the design region, so the prior bounds must contain the posterior, which the hold-out check and the prior support confirm here. The surrogate also adds its own approximation error, so its hold-out accuracy relative to the sampling error is what justifies replacing the model.
