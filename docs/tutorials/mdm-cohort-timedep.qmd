...
The base case reproduces the published table. Age-varying mortality lowers life expectancy, so every strategy accrues fewer QALYs than in the time-independent model. Strategy A is dominated; Strategy B and Strategy AB stay on the frontier.
...
Strategy B has an ICER near 65,000 per QALY and Strategy AB near 104,000, matching the published results. The analysis layer is identical to the time-independent case.
