...
This tutorial extends the [cohort state-transition tutorial](mdm-cohort.qmd) with age-varying mortality and transition rewards, reproducing the time-dependent Sick-Sicker tutorial of Alarid-Escudero and others (2023). The full script is [`examples/mdm_cohort_timedep.py`](https://github.com/pedroliman/heval/blob/main/examples/mdm_cohort_timedep.py).

## Two kinds of time dependence

Background mortality now rises with age. The `build` callback returns a per-cycle transition array of shape `(n_cycles, n_states, n_states)` instead of one matrix: the Healthy-to-Dead rate follows a US life table, and the Sick and Sicker states scale it by their hazard ratios. Transition rewards attach a one-time cost of dying, a cost of onset, and a disutility of onset to the flows between states, passed as `transition_cost` and `transition_effect`. The engine accrues state rewards on the occupancy trace and transition rewards on the flow, then discounts both with the same weights.
...
engine = MarkovModel(states=STATES, strategies=STRATEGIES, model_fn=model,
                            n_cycles=N, start="H", half_cycle_correction="simpson")
```

## Deterministic replication

The base case reproduces the published table. Age-varying mortality lowers life expectancy relative to the constant-rate model, so all strategies accrue fewer QALYs than in the introductory tutorial. Strategy A is dominated; Strategy B and Strategy AB stay on the frontier.
...
Strategy B has an ICER near 65,000 per QALY and Strategy AB near 104,000 per QALY, matching the published results. A per-cycle transition array is all the model structure this needs; the analysis layer is identical to the time-independent case.
