...
`as_outcomes` accepts a `DataFrame` or a CSV path with `strategy`, `iteration`, `cost`, and effect columns, and returns `Outcomes`, the structure every analysis function below consumes. Converting once here means `icer_table`, the acceptability curves, the value-of-information estimators, and the plots all take the same object, regardless of where the numbers originally came from.
...
`icer_table` orders strategies by cost and reports incremental cost-effectiveness ratios along the efficiency frontier.
...
The acceptability curves plot the same probabilities computed above as one line per strategy against the threshold, making the crossover in the decision visible at a glance.
