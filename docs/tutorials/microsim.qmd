...
description: "Build an individual-level model with `MicrosimModel`, for outcomes that depend on patient history or on heterogeneity a cohort average removes."
keywords: [microsimulation, individual-level simulation, patient-level simulation, individual sampling, cost-effectiveness modeling, health economics, Python]
---
...
The model structure goes to the engine when you build it; the draw matrix goes to `run_psa`. The two interventions are bare names, and the model functions branch on the intervention name (`intervention == "Treatment"`) to decide whether treatment applies. The two interventions share one population through common random numbers (the engine default), so the incremental result reflects the treatment effect rather than sampling noise.
...
This run passes `sequential=True` because a run this small finishes quickly; larger runs are parallel by default, and the two give identical results.
...
A microsimulation estimates each expected outcome as the mean over the $N$ simulated individuals, $\bar X = \frac{1}{N}\sum_{i=1}^{N} X_i$, whose Monte Carlo standard error falls as $\sigma/\sqrt{N}$: quadrupling the population halves it. With constant transitions and no heterogeneity, that mean must converge to the exact cohort calculation as the population grows; this section checks that it does, since a mismatch here would point to a bug rather than sampling noise. The engine and the cohort share the same discounting and half-cycle convention, so the only expected gap is that Monte Carlo error.
...
`MicrosimModel.continuous` is set up the same way. Instead of per-cycle probabilities, `event_times` returns sampled times to each competing event; the engine takes the earliest, advances to it, and accrues continuously between events, valuing each segment through `state_reward_rates` (per-year flows rather than per-cycle amounts). On constant hazards it reproduces the exponential cohort solution.
