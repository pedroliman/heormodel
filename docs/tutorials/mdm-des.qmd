...
The engine calls two functions the `model` module builds: `event_times`, which returns the sampled time to each competing transition from the current state, and `state_reward_rates`, which returns the annual cost and utility of each state. The article's next-event kernel is exactly the continuous clock of `MicrosimModel`: sample a latent time for each permitted transition, take the earliest, move, repeat. Because competing times are redrawn at every state entry, the Weibull progression draw needs no truncation and each death time reflects the individual's current age and state.
...
The transition amounts must be added on every iteration, so `costs_and_utilities_model` wraps the engine as a stochastic engine that collects the event history and folds the amounts in. `run_psa` drives that wrapper over the draw matrix, supplying the seed; one draw per experiment keeps each worker's event history small. The article runs 1,000 parameter sets of 100,000 individuals each; 300 sets of 2,500 individuals keep this page fast.
