...
This page reproduces the Sick-Sicker microsimulation of Krijkamp and others (2018) with `MicrosimModel`. It assumes the [microsimulation engine tutorial](microsim.qmd). Full script: [`examples/mdm_microsim.py`](https://github.com/pedroliman/heormodel/blob/main/examples/mdm_microsim.py).
...
The base-case parameters drive both arms through common random numbers with 50,000 individuals (the source used 100,000).
...
The published Table 3 reports no-treatment 62,667 dollars and 15.28 QALYs, treatment 117,455 dollars and 15.79 QALYs, and an ICER of 107,986 dollars per QALY. The sampler differs from the source, so the totals agree within Monte Carlo error rather than to the dollar and converge as the population grows.
