...
This tutorial mixes two parameter sources in one analysis: some parameters are calibrated to observed data, the rest come from the literature. Both feed one PSA that flows through CEA and VoI. It builds on the [probabilistic sensitivity analysis tutorial](psa.qmd) and is the narrated version of [`examples/calibration_workflow.py`](https://github.com/pedroliman/heormodel/blob/main/examples/calibration_workflow.py). Calibration needs the `calibration` extra: `uv pip install 'heormodel[calibration]'`.
...
outcomes = run_psa(disease_model, draws, sequential=True)
...
`capture_run` takes a `draw_sources` map so the run report records where each parameter came from.
...
Next: [parameter inputs from data](parameter-inputs.qmd) covers the base-case, imported, and posterior draw matrices that feed the same run loop.
