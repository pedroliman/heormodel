...
The epidemic and cost parameters can be set in a `ParameterSet`; in this example we set the reproduction number, rates, and costs using log-normal distributions, and the infectious-state utility takes a beta distribution bounded to the unit interval. The reproduction number carries the widest spread because it is the least certain.

Like with the other models, we define the parameter set, take a sample from those, generate outcomes with `run_psa` and then we can get our icer table:
...
All value of information functinos work with this moodel's output, including `evpi`:
