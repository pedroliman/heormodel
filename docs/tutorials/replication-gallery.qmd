...
| [Discrete-event simulation](mdm-des.qmd) | `MicrosimModel`, continuous clock | Matches the published figures, with two documented divergences |

The discrete-event source publishes figures rather than result tables. Its page reproduces the epidemiological curves and the frontier structure, and explains the two places the numbers depart from the published figures: transition rewards paid once at the event rather than accrued over the preceding sojourn, and a probabilistic analysis that draws every published distribution.
