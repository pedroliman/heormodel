...
This page collects three published Sick-Sicker cost-effectiveness tutorials, reproduced with `heormodel`, one per model type. Each replication first matches the source's deterministic results, then runs the same model through `heormodel.cea` and `heormodel.voi` to show how a probabilistic analysis extends it.
