...
def transition_probabilities(p, strategy, state, attrs, rng):
...
def state_rewards(p, strategy, state, attrs):
...
def microsim(n, var):
    return MicrosimModel.discrete(
        ...
        n_individuals=n, strategies=["Standard of care"], n_cycles=N_CYCLES,
        discount_rate=0.03, cycle_correction="half_cycle")

cohort = MarkovModel(states=STATES, strategies=("Standard of care",),
...
```{python}
def hist_transition(p, strategy, state, attrs, rng):
...
hist = MicrosimModel.discrete(
    ...
    n_individuals=40_000, strategies=["Standard of care"], n_cycles=N_CYCLES,
    discount_rate=0.03, cycle_correction="half_cycle",
